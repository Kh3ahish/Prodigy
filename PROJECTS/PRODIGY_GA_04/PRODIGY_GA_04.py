...
def generator_loss(generated_image, target):
    return tf.reduce_mean(tf.keras.losses.mean_squared_error(target, generated_image))

def discriminator_loss(real_validity, fake_validity):
    real_loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(tf.ones_like(real_validity), real_validity))
    fake_loss = tf.reduce_mean(tf.keras.losses.binary_crossentropy(tf.zeros_like(fake_validity), fake_validity))
